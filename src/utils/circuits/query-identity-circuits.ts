...
export class QueryIdentityCircuit {
  ...
  static TEST_DATA = {
    event_id: '0x123', // Math.random() до 255б
    event_data: '0x123321', // Math.random() до 255б
    id_state_root: '0x2e364a232db953191a2b3fe96fac9f44cfc7672ff2740666b61acde3304be2de', // circuit registry (icaoRoot)
    selector: '262143',
    ...
    expiration_date_upperbound:
      '21888242871839275222246405745257275088548364400416034343698204186575808495616', // timestemp
    citizenship_mask: '0x20000000000000000000000000', // valid Iran
    sk_identity: '0x1234567890', // circuit registry
    pk_passport_hash: '0x03a26b3d1c4155149447a34600cd510e48202e3ebf5eeb5487d688fb21cf1e8a', // registration proof output
    dg1: [
    ...
    // circuit registry (inclusion_branches)
    siblings: [
      '0x00',
      '0x00',
      '0x00',
  ...
  _ensureHexPrefix = (val: string) => (val.startsWith('0x') ? val : `0x${val}`)

  _getRandomHex(bits = 250) {
  ...
  buildInputs({
    ...
    // The circuit expects the `dg1` input to have exactly 108 elements.
    // If the input array is shorter, we pad it with zeros to meet the required length.
    const paddedDG1 = [...dg1.slice(0, 108), ...Array(Math.max(0, 108 - dg1.length)).fill(0)]

    return {
      event_id: this._getRandomHex(),
      event_data: this._getRandomHex(),
